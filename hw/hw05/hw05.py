...
class VendingMachine:
    ...
    def vend(self):
        ...
        elif self.deposit_ < self.price:
            return 'You must deposit $' + str(self.price - self.deposit_) + ' more.'
        elif self.deposit_ == self.price:
            self.number -= 1
            self.deposit_ -= self.price
            return 'Here is your {0}.'.format(self.product)
        else:
            self.number -= 1
            self.change = self.deposit_ - self.price
            self.deposit_ = 0
            return 'Here is your {0} and ${1} change.'.format(self.product, self.change)
    def deposit(self,money):
        if self.number == 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(money)
        else:
            self.deposit_ += money
            return 'Current balance: ${0}'.format(self.deposit_)
    def restock(self,addin):
        self.number += addin
